...
@Slf4j
public class MerchandiseLogServiceStrategy implements LogServiceStrategy {
    public static final String MERCHANDISE_LOGS_FILE_LOCATION = "C:\\\\logs\\merchandise";
    public static final String LOG_FORMAT = ".log";

    @Override
    public byte[] getLogs(Date date) {
        try {
            InputStream in = new FileInputStream(MERCHANDISE_LOGS_FILE_LOCATION + apply(date) + LOG_FORMAT);
