...
@Slf4j
public class ProductLogServiceStrategy implements LogServiceStrategy {
    public static final String PRODUCT_LOGS_FILE_LOCATION = "C:\\\\logs\\product";
    public static final String LOG_FORMAT = ".log";

    @Override
    public byte[] getLogs(Date date) {
        try {
            InputStream in = new FileInputStream(PRODUCT_LOGS_FILE_LOCATION + apply(date) + LOG_FORMAT);
