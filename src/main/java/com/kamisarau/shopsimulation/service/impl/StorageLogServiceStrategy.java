...
@Slf4j
public class StorageLogServiceStrategy implements LogServiceStrategy {
    public static final String STORAGE_LOGS_FILE_LOCATION = "C:\\\\logs\\storage";
    public static final String LOG_FORMAT = ".log";

    @Override
    public byte[] getLogs(Date date) {
        try {
            InputStream in = new FileInputStream(STORAGE_LOGS_FILE_LOCATION + apply(date) + LOG_FORMAT);
