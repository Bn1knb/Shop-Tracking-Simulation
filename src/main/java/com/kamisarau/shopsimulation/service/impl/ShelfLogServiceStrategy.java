...
@Slf4j
public class ShelfLogServiceStrategy implements LogServiceStrategy {
    public static final String SHELF_LOGS_FILE_LOCATION = "C:\\\\logs\\shelf";
    public static final String LOG_FORMAT = ".log";

    @Override
    public byte[] getLogs(Date date) {
        try {
            InputStream in = new FileInputStream(SHELF_LOGS_FILE_LOCATION + apply(date) + LOG_FORMAT);
